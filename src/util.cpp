...
const char * const PIVX_CONF_FILENAME = "lunariumcoin.conf";
const char * const PIVX_PID_FILENAME = "lunariumcoin.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";
const char * const PIVX_ACTIVE_MASTERNODE_CONF_FILENAME = "activemasternode.conf";


// LunariumCoin only features
// Masternode
...
#else
    const char* pszModule = "lunariumcoin";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\lunariumcoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\lunariumcoin
// Mac: ~/Library/Application Support/lunariumcoin
// Unix: ~/.lunariumcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LunariumCoin";
#else
    fs::path pathRet;
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty lunariumcoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override lunariumcoin.conf
