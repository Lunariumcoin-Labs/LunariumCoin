...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("LunariumCoin client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("LunariumCoin"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("LunariumCoin"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
