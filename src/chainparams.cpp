...
public:
    CMainParams()
    {
        ...
        // Default P2PKH/P2SH stay 'X'/… (bytes 75/78). From nAddressPrefixV2ActivationHeight onward, encoding uses V2 ('L'/…).
        // Set height to -1 to keep legacy display only; decoding still accepts V2 when prefixes differ (e.g. treasury mint string).
        nAddressPrefixV2ActivationHeight = 1280000;
        for (unsigned int t = 0; t < CChainParams::MAX_BASE58_TYPES; t++)
            base58PrefixesV2[t] = base58Prefixes[t];
        base58PrefixesV2[PUBKEY_ADDRESS] = std::vector<unsigned char>(1, 48);  // 'L'
        // P2SH prefix unchanged from legacy (78); only P2PKH display switches to 'L'.

        // Optional one-block mint for migration/exchange (same pubkey as an 'L' address you import in a wallet):
        consensus.nXLNCoinSupplyMintHeight = 1270000;
        consensus.nXLNCoinSupply = 47000000 * COIN;
        consensus.sXLNCoinSupplyMintAddress = "LRdqugAgypEzDVyp1cJ86bwDrNW12BDefT";  // DecodeDestination accepts L before the display fork height.
...
public:
    CTestNetParams()
    {
        ...
        nAddressPrefixV2ActivationHeight = -1;
        for (unsigned int t = 0; t < CChainParams::MAX_BASE58_TYPES; t++)
            base58PrefixesV2[t] = base58Prefixes[t];
...
const std::vector<unsigned char>& CChainParams::Base58PrefixForAddressEncoding(Base58Type type, int nHeight) const
{
    if (type != PUBKEY_ADDRESS && type != SCRIPT_ADDRESS)
        return base58Prefixes[type];
    if (nAddressPrefixV2ActivationHeight >= 0 && nHeight >= nAddressPrefixV2ActivationHeight)
        return base58PrefixesV2[type];
    return base58Prefixes[type];
}
