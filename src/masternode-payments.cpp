...
bool IsBlockPayeeValid(const CBlock& block, int nBlockHeight)
{
    ...
    // if it's the XLN coin supply mint block then verify if exists an output with the right amount and address
    if (consensus.nXLNCoinSupplyMintHeight == nBlockHeight) {
        LogPrint(BCLog::MASTERNODE, "masternode", "IsBlockPayeeValid: Check XLN coin supply mint reward\n");
        
        CAmount amount = CMasternode::GetBlockValue(nBlockHeight) - CMasternode::GetBlockValue(nBlockHeight + 1);
        CScript payee = GetScriptForDestination(DecodeDestination(consensus.sXLNCoinSupplyMintAddress));

        LogPrint(BCLog::MASTERNODE, "IsBlockPayeeValid, expected XLN coin supply mint amount is %lld, coins %f\n", amount, (float)amount / COIN);

        bool fXLNCoinSupplyMintFound = false;
        for(CTxOut out : txNew.vout) {
            if (payee == out.scriptPubKey && amount == out.nValue) {
                fXLNCoinSupplyMintFound = true;
            }
        }

        if(!fXLNCoinSupplyMintFound) return false;
    } 
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, const CBlockIndex* pindexPrev, bool fProofOfStake)
{
    ...
    CAmount xlnCoinSupplyMint = 0;
    CScript xlnPayee;

    // XLN coin supply mint
    if (consensus.nXLNCoinSupplyMintHeight == nHeight) {
        xlnCoinSupplyMint = consensus.nXLNCoinSupply;
        xlnPayee = GetScriptForDestination(DecodeDestination(consensus.sXLNCoinSupplyMintAddress));
    }

    //subtract mn payment from the stake reward plus the XLN coin supply mint 
    CAmount reductionAmount = masternodePayment + xlnCoinSupplyMint;
    ...
    if (hasPayment) {
        ...
        // removes the XLN coin supply mint if there is no masternode to pay
        if (fProofOfStake) {
            txNew.vout[txNew.vout.size() - 1].nValue = blockValue - xlnCoinSupplyMint;
        } else {
            txNew.vout[0].nValue = blockValue - xlnCoinSupplyMint;
        }
    }

    // Adds XLN coin supply mint
    if(xlnCoinSupplyMint > 0) {
        unsigned int i = txNew.vout.size();
        txNew.vout.resize(i + 1);
        txNew.vout[i].scriptPubKey = xlnPayee;
        txNew.vout[i].nValue = xlnCoinSupplyMint;
    }
}
